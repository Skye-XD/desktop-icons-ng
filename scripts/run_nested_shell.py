...
Extension bundle, specified by `--bundle` argument, will be automatically
...
def run_with_basedir(*, base_dir, **kwargs):
    if base_dir is None:
        with tempfile.TemporaryDirectory() as tempdir:
            run_shell_devkit(env=make_env(tempdir), **kwargs)

    else:
        run_shell_devkit(env=make_env(base_dir), **kwargs)
...
def main():
    ...
    if default_pack:
        ...
        default_pack = next(pathlib.Path.cwd().glob('**/gtk4-ding@smedius.gitlab.com.zip'), None)
