...
class MetricsWidget {
    ...
    async start() {
        ...
        this.c.onBackendEvent((n, p) => {
            if (n === 'metrics')
                this.apply(p.snapshot);
        });
        this.c.onVisibilityChange?.(visible => {
            if (visible && this._lastSnapshot)
                this.apply(this._lastSnapshot);
        });
        try {
    ...
    _applyConfig() {
        ...
        const rawColor = typeof this._config.color === 'string' ? this._config.color.trim() : '';
        const hex = /^#([0-9a-f]{6})$/i.test(rawColor)
            ? rawColor
            : map[rawColor.toLowerCase()] || map.cyan;
        document.body?.style?.setProperty('--neon', hex);

        const rawBg = typeof this._config.bgColor === 'string' ? this._config.bgColor.trim() : '';
        const bgColor = /^#([0-9a-f]{6})$/i.test(rawBg)
            ? rawBg
            : map[rawBg.toLowerCase()] || hex;
        document.body?.style?.setProperty('--bg-color', bgColor);

        const rawAlpha = Number(this._config.bgAlpha);
        const bgAlpha = Number.isFinite(rawAlpha) ? Math.max(0, Math.min(0.35, rawAlpha)) : 0.06;
        document.body?.style?.setProperty('--bg-alpha', String(bgAlpha));

        const rawText = typeof this._config.textColor === 'string' ? this._config.textColor.trim() : '';
        const textColor = /^#([0-9a-f]{6})$/i.test(rawText)
            ? rawText
            : '#ffffff';
    ...
    apply(s) {
        ...
        const setBar = (key, selector, pRaw) => {
            const t = document.querySelector(selector);
            if (!t) {
                console.warn('metrics widget missing track', selector);
                return;
            }
            ...
            if (edge) {
                ...
                edge.style.left = `${(p * 100).toFixed(1)}%`;
            }
        };
        ...
        if (cpuNum)
            cpuNum.textContent = `${cpuPct.toFixed(0)}%`; else
            console.warn('metrics widget missing cpu num');

        // RAM
        let ramPct = 0;
        let ramCachePct = 0;
        if (s.mem?.totalBytes)
            ramPct = clamp01(s.mem.usedBytes / s.mem.totalBytes);
        if (s.mem?.totalBytes)
            ramCachePct = clamp01((s.mem.cachedBytes ?? 0) / s.mem.totalBytes);
        ...
        if (ramNum) {
            ...
            console.warn('metrics widget missing ram num');
        }
        ...
        const scaleFor = v => {
            ...
            if (base >= 1e9)
                return {barScale: base, displayDiv: 1e9, suffix: 'G'};
            if (base >= 1e6)
                return {barScale: base, displayDiv: 1e6, suffix: 'M'};
        ...
        if (rxNum)
            rxNum.textContent = fmt(rx, rxScale); else
            console.warn('metrics widget missing rx num');
        if (txNum)
            txNum.textContent = fmt(tx, txScale); else
            console.warn('metrics widget missing tx num');
        ...
        if (batNum) {
            ...
            console.warn('metrics widget missing bat num');
        }
    ...
    _applyNumberWidths() {
        ...
        for (const [row, width] of Object.entries(map)) {
            const el = document.querySelector(`.row[data-row="${row}"]`);
            if (el)
                el.style.setProperty('--num-width', width);
        }
    }
