...
(() => {
  ...
  function parseOffsetToMinutes(raw) {
    ...
    if (s.length <= 2) {
      const hh = Number(s);
      if (!Number.isInteger(hh) || hh > 14) return null;
      return sign * (hh * 60);
    }
  ...
  function debounce(timerRef, ms, fn) {
    if (timerRef.value) clearTimeout(timerRef.value);
    timerRef.value = setTimeout(fn, ms);
  }
  ...
  async function init() {
    ...
    try {
      if (window.ding && typeof window.ding.getConfig === 'function') {
        ...
        if (cfg && typeof cfg === 'object') config = { ...DEFAULT_CONFIG, ...cfg };
      }
    } catch (e) {}
